...
const getSchedules = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'giangvien' || req.user.role === 'nhanvien') {
      ...
      const scopedUserIds = await User.find({ departmentId: { $in: departmentIds } }).distinct('_id');
      if (userId && !scopedUserIds.some((id) => id.toString() === userId.toString())) {
        return sendError(res, 'Bạn chỉ có quyền xem lịch thuộc khoa của mình.', null, 403, ERROR_CODES.AUTH_FORBIDDEN);
      }
      query.userId = userId ? userId : { $in: scopedUserIds };
    } else if (userId) {
      // Admin có thể xem toàn trường hoặc lọc theo người
      query.userId = userId;
    }
...
const getScheduleById = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'truongkhoa' && !(await isUserInDeanScope(req.user, schedule.userId?._id || schedule.userId))) {
      return sendError(res, 'Bạn chỉ có quyền xem lịch thuộc khoa của mình.', null, 403, ERROR_CODES.AUTH_FORBIDDEN);
    }
...
const createSchedule = async (req, res, next) => {
  try {
    ...
    const assignedUser = await User.findById(userId).select('role');
    if (!assignedUser) {
      return sendError(
        res,
        'Người được phân công không tồn tại trên hệ thống.',
        null,
        404,
    ...
    if (!(await isUserInDeanScope(req.user, userId))) {
      return sendError(res, 'Bạn chỉ có quyền phân lịch cho nhân sự thuộc khoa của mình.', null, 403, ERROR_CODES.AUTH_FORBIDDEN);
    }

    const canTeach = assignedUser.role === 'giangvien' || assignedUser.role === 'truongkhoa';
    if (!canTeach && (subjectName?.trim() || subjectCode?.trim())) {
      return sendError(
        res,
        'Nhân viên hành chính không được gắn môn học hoặc mã môn học.',
        null,
        400,
        ERROR_CODES.SCHEDULE_INVALID_DATA
      );
...
const updateSchedule = async (req, res, next) => {
  try {
    ...
    const assignedUser = await User.findById(targetUserId).select('role');
    if (!assignedUser) {
      return sendError(
        res,
        'Người được phân công không tồn tại trên hệ thống.',
    ...
    const updatePayload = {
      ...
      ...(canTeach && subjectName !== undefined ? { subjectName: subjectName.trim() || '' } : {}),
      ...(canTeach && subjectCode !== undefined ? { subjectCode: subjectCode.trim().toUpperCase() || '' } : {}),
      ...(!canTeach ? { subjectName: '', subjectCode: '' } : {}),
...
const deleteSchedule = async (req, res, next) => {
  try {
    const schedule = await Schedule.findById(req.params.id).select('userId');
    if (!schedule) {
      return sendError(
        res,
    ...
    if (!(await isUserInDeanScope(req.user, schedule.userId))) {
      return sendError(res, 'Bạn chỉ có quyền xóa lịch thuộc khoa của mình.', null, 403, ERROR_CODES.AUTH_FORBIDDEN);
    }
  ...
  try {
    ...
    if (req.user && (req.user.role === 'giangvien' || req.user.role === 'nhanvien')) {
      ...
      const scopedUserIds = new Set(
        (await User.find({ departmentId: { $in: departmentIds } }).distinct('_id')).map((id) => id.toString())
      );
      if (req.query.userId && !scopedUserIds.has(req.query.userId.toString())) {
        return sendError(res, 'Bạn chỉ có quyền xem lịch thuộc khoa của mình.', null, 403, ERROR_CODES.AUTH_FORBIDDEN);
      }
      filteredSchedules = schedules.filter(
        (s) => s.userId && scopedUserIds.has(s.userId._id.toString()) && (!req.query.userId || s.userId._id.toString() === req.query.userId.toString())
      );
